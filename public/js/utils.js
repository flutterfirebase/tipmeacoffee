        ...
        $.ajax({url: '/', type: 'GET', data: { index: index },contentType: 'application/json',
            success: function(data) {
                ...
                data.articles.forEach(function(article) { //console.log('Article=>',article);
                    ...
                    if(article.json && article.json.type){var hashregex = /(^|\B)#(?![0-9_]+\b)([a-zA-Z0-9_]{1,30})(\b|\r)/g;var content=article.json.body;var ptags = content.match(hashregex);if(ptags){for(var i = 0, l = ptags.length; i < l; i++ ){content=content.replace(ptags[i], '<a style="color:rgb(0 43 255)" href="/tags/'+ptags[i].replace(/#/g, '')+'">'+ptags[i]+'</a>');};}
