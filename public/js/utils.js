...
$('.btn_follow_catg').click(function() {
    let $this=$(this);var catgName = $(this).attr("data-catg");$(this).html('Following...').attr("disabled", true);
...
$('.btn_unfollow_catg').click(function() {
    let $this=$(this);var catgName = $(this).attr("data-catg");$(this).html('UNFollowing...').attr("disabled", true);
...
$('.btn_app_wit').click(function() {
    var nodeName = $(this).attr("data-node");$(this).closest("tr").find(".btn_txt_app").html('Approving..');
...
$('.btn_unapp_wit').click(function() {
    var nodeName = $(this).attr("data-node");$(this).closest("tr").find(".btn_txt_unapp").html('...');
            ...
            success: function(data) {
                ...
                data.articles.forEach(function(article) { //console.log('Article=>',article);
                    ...
                    if(article.json && article.json.type){var hashregex = /(^|\B)#(?![0-9_]+\b)([a-zA-Z0-9_]{1,30})(\b|\r)/g;var content=article.json.body;var ptags = content.match(hashregex);if(ptags){for(var i = 0, l = ptags.length; i < l; i++ ){content=content.replace(ptags[i], '<a style="color:rgb(0 43 255)" href="/tags/'+ptags[i].replace(/#/g, '').toLowerCase()+'">'+ptags[i]+'</a>');};}
                        if(article.json.type==3){postBody = '<a class="post-content" data-permlink="'+article.link+'" data-author="'+article.author+'" href="/post/'+article.author+'/'+article.link+'"><p>'+content+'</p></a>';
                        }else if(article.json.type==2){postBody = '<a class="post-content" data-permlink="'+article.link+'" data-author="'+article.author+'" href="/post/'+article.author+'/'+article.link+'">'+content+'</p></a><div class="card-content-images"><a href="/post/'+article.author+'/'+article.link+'"><div class="card-image-link"><img alt="'+article.link+'" src="'+article.json.image+'" /></div></a></div>';
