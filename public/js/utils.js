    ...
    if (breeze_username) {let input_url = $("#url_field").val(); if (!input_url){ $("#url_field").css("border-color", "RED");toastr.error('phew... You forgot to enter URL');$('#share_plus').html('Share');$(".share_me").attr("disabled", false);return false;}
        $.ajax({url: '/share',type: 'POST',data: JSON.stringify({ url: input_url }),contentType: 'application/json',
            success: function(data)  {
                ...
                } else {meta=data.meta;$('.share_link').hide();$('.edit_psot').show();$('.data-title').html(meta.title);$(".link_image").attr("src", meta.image);$('#domain_name').html(data.link);$('#post_desc').html(trimString(meta.description,120));$('.url_link').val(input_url); }
            }
        });
...
$('.share_new_post').click(function(e) {
    if (breeze_username) {e.preventDefault();let urlInput = $('.url_link').val();
        if($('#share_cat').val() == "0") {$('#share_cat').css("border-color", "RED");toastr.error('Select an appropriate Category');return false;}
        let inputtags = $.trim($('.share_tags').val()).toLowerCase();if(inputtags == "") {$('#share_cat').css("border-color", "RED");toastr.error('Add 2-5 related tags');return false;}
        ...
        $.ajax({type: "POST",url: "/post",data: {title: title,tags:inputtags,description:post_body,category: category,image:urlImage,exturl:urlInput},
            success: function(data) {if (data.error == false) {toastr['success']("Link Shared Successfully!");setTimeout(function(){window.location.href = '/';}, 200); } else {toastr['error'](data.message);$(".share_new_post").attr("disabled", false);$('.edit_post_txt').html('Publish');return false} }
        });
...
$('.Home-wrapper, .profile-wrapper').on("click", ".card-icon", function() {
    if (breeze_username) {var $this = $(this);var postLink = $this.attr("data-permlink");var postAuthor = $this.attr("data-author");$this.addClass('hov_ani');
        $.ajax({ type: "POST",url: "/upvote", data: {author: postAuthor, postLink: postLink}, success: function(data) {
            if (data.error == false) {$this.removeClass('hov_ani');let likes=$this.parent().find('.card-icon-value').html();let nLikes=parseInt(likes)+1;$this.addClass('hov_done');$this.parent().find('.card-icon-value').css("color", "#e0245e").html(nLikes);let post_income=$this.closest('.likes_section').find('.post-income').html();let nIncome=(parseFloat(post_income)+0.10).toFixed(2);$this.closest('.likes_section').find('.post-income').html(nIncome);toastr['success']("Upvoted Successfully!");//setTimeout(function(){window.location.reload();}, 400);
