...
$('.add_post').click(function() {
  ...
  $.ajax({url: '/postlinks',type: 'POST',contentType: false,processData: false,data: formData,
    success: function(data)  {
      if (data.error == false) {$('.add_post_txt').html('Posting...');$(".add_post").attr("disabled", true);toastr['success']("Published Successfully!");setTimeout(function(){window.location.href = '/post/'+data.author+'/'+data.link;}, 200); 
