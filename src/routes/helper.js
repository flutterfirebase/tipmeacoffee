...
const getTags = async (maxTags) => { if(!maxTags) maxTags = 6; let timeNow = new Date().getTime(); let postsTime = timeNow - 7200000; let tagsAPI = await axios.get(api+`/trending?after=${postsTime}&limit=100`); let posts = tagsAPI.data; let tags = {};
