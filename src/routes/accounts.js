        ...
        breej.getAccounts([inputName], function (error, accounts) {
            if (!accounts || accounts.length === 0) {
                db.collection('users').findOne({ email: uEmail }, function (err, user) {
                    if (user) {
                        res.send({ error: true, message: 'phew... Email is already in use' });
                    } else {
                        const crypto = require('crypto'); const vtoken = crypto.randomBytes(16).toString('hex');
    ...
    try {
        ...
        if (token && await validateToken(req.cookies.breeze_username, req.cookies.token)) {
            ...
            db.collection('users').findOne({ token: vtoken }, function (err, result) {
                if (result && result.isvarified == false) {
                    v_token = 'valid', username = result.username.toLowerCase()
                    ...
                    if (result) {
                        ...
                        breej.sendTransaction(signedTx, (error, result) => {
                            if (error === null) {
                                ...
                                let newBTx = { type: 15, data: { receiver: post.name, amount: parseInt(3600) } };
...
async function verifyMail(rAddress, message) {
    let transporter = nodemailer.createTransport({
        host: process.env.TRANSMAIL_HOST, port: 465, secure: true,
